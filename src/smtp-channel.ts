...
export class SMTPChannel {
    ...
    public negotiateTLS(config: any) {
        return promiseWithTimeout({
          time: config.timeout,
    ...
    public parseReplyCode(line: any) {
        return line ? line.substr(0, 3) : null;
    }

    public  isLastReply(line: any) {
        return line ? line.charAt(3) === ' ' : null;
    }

    private _createSocket(config: any, onConnect: any) {
    ...
    private _connectAsPromised(handler?: any) {
    ...
    private _writeAsPromised(data: any, handler: any) {
        return new Promise((resolve, reject) => {
          if (!this._socket) {
    ...
    private _negotiateTLSAsPromised(config: any) {
        return new Promise((resolve, reject) => {
    ...
    private _resolveCommand(resolve: any, reject: any, handler: any) { // handling request
    ...
    private _convertToStream(data: any) {
        if (data.pipe) {
          return data;
        }
    ...
    private _onReply(line: any) {
        this.replySubject.next(line);
    }

    private _onSend(chunk: any) {
